...
#define xexpansion 0.15
#define yexpansion 0.15
...
#define individual_fiber_props true
...
void read_circles_individual(Mesh* mesh, std::string filename, std::vector<double>& maxes, std::vector<double>& mins, double& min_rad, id_type& dimension,
							 std::vector<double>& sigma_c, std::vector<double>& delta_c)
{
	// Currently only support reading in 2D fiber geometries
	dimension = 2;
	maxes = {-99999999999999999999999999., -99999999999999999999999999.};
	...
	while (myfile.peek() != EOF)
	{
		...
		// update the sigmac and deltac arrays
...
int main (int argc, char* argv[])
{
	...
	if (!subscale_flag)
	{
		...
		{
			if (!individual_fiber_props)
				read_circles(&mesh, filename, maxes, mins, min_rad, dimension);
			else
				read_circles_individual(&mesh, filename, maxes, mins, min_rad, dimension, sigma_c, delta_c);
		}
		...
		if (argc == 2) // Generate mesh using given parameters
		{
		...
		else if (argc == 3) // Second command line parameters is ana abaqus input file name
		{
		...
		problem->addMaterialSensitivityParameter("Epoxy-Carbon0", "sigma");
		problem->addMaterialSensitivityParameter("Epoxy-Carbon0", "delta");
		problem->addMaterialSensitivityParameter("Epoxy-Carbon1", "sigma");
		problem->addMaterialSensitivityParameter("Epoxy-Carbon1", "delta");
		problem->init();
		...
		std::vector<double> start = {0.0025980762, 0.0015};
		std::vector<double> end = {0.003550742, 0.00205};
		std::string path_name = "Output/path.csv";
		problem->outputCohesiveOpening("Output/coh_openings.csv");
		problem->outputPathSolution(path_name, start, end, 100, true);
