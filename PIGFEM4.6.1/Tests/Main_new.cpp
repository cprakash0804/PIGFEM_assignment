...
#define n_elem_per_inclusion 11	// Define the number of elements to put across the diameter of the smallest fiber
#define xexpansion 0.1
#define yexpansion 0.1
...
void add_circle(Mesh* mesh, Inclusion* ellipse, double x, double y, double r)
{
...
void add_sphere(Mesh* mesh, Inclusion* ellipsoid, double x, double y, double z, double r)
{
	std::vector<double> center = {x, y, z};
	ellipsoid->set_vec_parameter("center", center);
	ellipsoid->set_parameter("a", r);
	ellipsoid->set_parameter("b", r);
	ellipsoid->set_parameter("c", r);
...
void read_circles(Mesh* mesh, std::string filename, std::vector<double>& maxes, std::vector<double>& mins, double& min_rad, id_type& dimension)
{
	...
	while (myfile.peek() != EOF)
	{
		...
		if (!(c == '\n' || c == '\r') && myfile.peek() != EOF) // This is reading in spheres instead of circles (so c should be a comma currently) (if its the end of a circle file c will still be a comma so need to check EOF)
		{
			z = r;
			myfile >> r;
			add_sphere(mesh, &ellipsoid, x, y, z, r);
			dimension = 3;
			if (maxes.size() == 2)
			{
			...
			add_circle(mesh, &ellipse, x, y, r); // Don't need to read in the endline already got it
...
int main (int argc, char* argv[])
{
	...
	if (!subscale_flag)
	{
		...
		std::vector<double> start = {0.003, 0.0};
		std::vector<double> end = {0.0041, 0.0};
