...
int main (int argc, char* argv[])
{
	...
	{
		std::set<id_type> curr_set = boundary->get_nodeset(sets[s]);
	...
	if(mesh.IGFEM())
	{
		...
		L2 = problem.Compute_L2_Norm(&Analytical_x_tension);

	// Output results to file
	if(rank==rank_see)
		cout << "\nOutputting results to file...";
	char buf[50];
	sprintf(buf, "Output/IGFEM2_out_%i", nelem);
	problem.writeToVTK( std::string(buf) );
