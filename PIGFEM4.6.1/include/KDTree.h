...
class KDTree {
	...
	private: std::vector<Point> points;    // Points data
	private: std::vector<KDNode*> nodesPtrs; // Memory to keep nodes
	private: int ndim;                // Data dimensionality
	private: int npoints;             // Number of points
	private: std::vector<int>   workarray; // Used in tree construction
...
//		}			
     ...
     *                 by the data
     */
	public: KDTree(const std::vector<Point>& points){
    	// initialize data
	    ...
	    std::vector< std::vector<int> > indexes( ndim, std::vector<int>(npoints,0) );
	    for( int dIdx=0; dIdx<ndim; dIdx++ )
	    ...
	    std::vector< std::vector<int> > srtidx( ndim, std::vector<int>(npoints,0) );
	 ...
	 */
	private: int build_recursively(std::vector< std::vector<int> >& sortidx, std::vector<int>& pidx, int dim){
		#ifdef DEBUG
		...
		// Stop condition
	  	if(pidx.size() == 1) {
	  		KDNode *node = new KDNode();		// create a new node
	...
	private: void linear_tree_print(){
		for (unsigned int i=0; i < nodesPtrs.size(); i++) {
			KDNode* n = nodesPtrs[i];
			std::cout << "[i]" << i << " key: " << n->key << " LIdx: "<< n->LIdx << " RIdx: "<< n->RIdx << std::endl; 
		}
	}
	...
	public: void left_depth_first_print( int nodeIdx = 0 ){
		KDNode* currnode = nodesPtrs[nodeIdx];
	...
	void print_tree( int index = 0, int level = 0 ){
		KDNode* currnode = nodesPtrs[index];
		
		// leaf
		if( currnode->pIdx >= 0 ){
			std::cout << "--- "<< currnode->pIdx+1 << " --- "; //node is given in matlab indexes
			for( int i=0; i<ndim; i++ ) std::cout << points[ currnode->pIdx ][ i ] << " ";
			std::cout << std::endl;
		}
		else
	 ...
	 * 
	 */
	public: void k_closest_points(const Point& Xq, int k, std::vector<int>& idxs, std::vector<double>& distances){
    	...
    	int N = pq.size();
    	for (int i=0; i < N; i++) {
			std::pair<double, int> topel = pq.top();
    		pq.pop();
    		if( i>=N-k ){
	...
	private: void leaves_of_node( int nodeIdx, std::vector<int>& indexes ){
		KDNode* node = nodesPtrs[ nodeIdx ];
		if( node->isLeaf() ){
	 ...
	 */
	public: void ball_query( const Point& point, const double radius, std::vector<int>& idxsInRange, std::vector<double>& distances ){
	 ...
	 */
	public: void ball_bbox_query(int nodeIdx, Point& pmin, Point& pmax, std::vector<int>& inrange_idxs, std::vector<double>& distances, const Point& point, const double& radiusSquared, int dim=0){
		KDNode* node = nodesPtrs[nodeIdx];
	 ...
	 * 
	 */
	public: void range_query( const Point& pmin, const Point& pmax, std::vector<int>& inrange_idxs, int nodeIdx=0, int dim=0 ){
		KDNode* node = nodesPtrs[nodeIdx];
